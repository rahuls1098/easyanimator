 ...
 * file containing the animation and builds a cs3500.animator.model It is parameterized over the
 * actual cs3500.animator.model type.
 */

public interface TweenModelBuilder<T> {

  /**
   * Add a new oval to the cs3500.animator.model with the given specifications.
   *
  ...
  /**
   * Add a new rectangle to the cs3500.animator.model with the given specifications.
   *
   * @param name        the unique name given to this shape
  ...
  /**
   * Rotate this shape by a specified number of degrees (anti-clockwise for positive) about
   * its center, from the start time to end time.
   * @param name        the unique name of the shape ot be rotated
   * @param fromTheta   the degree from which the shape is rotating
   * @param toTheta     the degree to which the shape is rotating
   * @param startTime   the time tick at which this rotate should start
   * @param endTime     the time tick at which this rotate should end
   */
  TweenModelBuilder addRotate(String name, float fromTheta, float toTheta,
                              int startTime, int endTime);

  /**
   * Return the cs3500.animator.model built so far.
   *
   * @return the cs3500.animator.model that was constructed so far
   */
  T build();
