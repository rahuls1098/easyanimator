package cs3500.animator.readbuild;
 ...
 * prescribed file format, and relies on a cs3500.animator.model builder interface. The user of this
 * class should create a cs3500.animator.model builder that implements this interface.
 */

public class AnimationFileReader {
  /**
   * READ ME:
   * Commented out "theta" fields and set/get method for shapes due to
   * design choice to not require specification of initial degree orientation of
   * shape in the input file. Default: 90 degrees orientation.
   */

  /**
   * Read the animation file and use the builder to build a cs3500.animator.model.
   *
   * @param fileName the path of the file to be read
   * @param builder  the builder used to build the cs3500.animator.model
   * @param <T>      the type of cs3500.animator.model
   * @return the cs3500.animator.model
      ...
      switch (command) {
        case "rectangle":
          ...
          builder.addRectangle(
                  ...
                  rinfo.getWidth(), rinfo.getHeight(), rinfo.getTheta()
        ...
        case "oval":
          ...
          builder.addOval(
                  ...
                  cinfo.getXRadius(), cinfo.getYRadius(), cinfo.getTheta(),
        ...
        case "rotate":
          ...
          builder.addRotate(rotateByInfo.name,
                  rotateByInfo.getFromTheta(),
                  rotateByInfo.getToTheta(),
  ...
  private RotateByInfo readRotateByInfo(Scanner sc) throws
    ...
    while(!info.isAllInitialized()) {
      ...
      switch(command) {
        case "rotateto" :
          info.setFromThetaRotate(sc.nextFloat());
          info.setToThetaRotate(sc.nextFloat());
          break;
        case "name" :
          info.setName(sc.next());
          break;
  ...
  class RotateByInfo extends Inputable {
    ...
    void setFromThetaRotate(float fTheta) {
      this.fromTheta = fTheta;
      valueFlags.replace("fromTheta", true);
    }

    void setToThetaRotate(float tTheta) {
      this.toTheta = tTheta;
